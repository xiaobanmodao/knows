...
Page({
  ...
  async onLoad(options = {}) {
    ...
    this.currentUnitId = options.id || this.currentUnitId || '';

    try {
      const unit = getEnglishUnitById(options.id);
      if (!unit) throw new Error('教材单元不存在');
      ...
      try {
        ...
        if (!this.pageActive || this.assetRequestToken !== requestToken || this.currentUnitId !== unit.id) return;
        this.setData({
      ...
      this.setData({ loading: false, unit: null, notFound: '当前英语单元暂未打开，请重试。' });
    }
  },
  ...
  reopen() {
    this.onLoad({ id: this.currentUnitId });
  },
