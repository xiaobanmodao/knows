...
Page({
  ...
  async loadKnowledge(knowledgeId) {
    ...
    try {
      ...
      if (!knowledge) throw new Error('生物知识点不存在');
      ...
      const note = app.getKnowledgeNote('biology', knowledge.id);
      ...
      app.addRecent({
        ...
        subtitle: context ? `生物 · ${context.title}` : '生物知识点',
        subjectId: 'biology',
        type: 'knowledge',
  ...
  openContext() {
    ...
    openBiologyContent({ type: 'topic', id: this.data.context.id });
  },
  ...
  openRelated(event) {
    ...
    if (!id) return;
    this.persistReadingPosition();
    openBiologyContent({ type: 'knowledge', id });
  },
  ...
  openAdjacent(event) {
    ...
    if (!id) return;
    this.persistReadingPosition();
    openBiologyContent({ type: 'knowledge', id }, { replace: true });
  },
  ...
  onCoverImageError(event) {
    console.warn('生物知识图加载失败', event.detail);
