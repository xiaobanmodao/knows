...
assert.strictEqual(report.totals.queued, 0, '当前应有 0 个未复核实体');
assert.strictEqual(report.items.length, 0, '复核队列条目数不正确');
assert.deepStrictEqual(report.totals.bySubject, { english: 0, math: 0, physics: 0 }, '学科队列数量不正确');
assert.strictEqual(report.totals.byType['english:template'], undefined, '英语方法模板不应继续进入复核队列');
assert.strictEqual(report.totals.byType['physics:template'], undefined, '物理章节方法模板不应继续进入复核队列');
assert.strictEqual(report.totals.byType['physics:structured-template'], undefined, '物理结构化方法模板不应继续进入复核队列');
...
assert.deepStrictEqual(report.totals.byPriority, {}, '复核队列优先级数量不正确');
