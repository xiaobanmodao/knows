...
assert.strictEqual(report.totals.queued, 28, '当前应有 28 个未复核实体');
assert.strictEqual(report.items.length, 28, '复核队列条目数不正确');
assert.deepStrictEqual(report.totals.bySubject, { english: 0, math: 0, physics: 28 }, '学科队列数量不正确');
...
assert.deepStrictEqual(report.totals.byPriority, { 2: 28 }, '复核队列优先级数量不正确');
