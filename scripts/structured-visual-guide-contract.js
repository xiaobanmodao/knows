...
function validateSourceGuides(subjectLabel, sourceKnowledgeItems) {
  const issues = [];
...
function compareRuntimeGuideLayer(subjectLabel, sourceKnowledgeItems, layer) {
  const issues = [];
...
function collectStructuredVisualGuideIssues({
  ...
  issues.push(...collectGuideRecords(sourceKnowledgeItems, '源数据').issues);
