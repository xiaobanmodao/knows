...
assert.ok(findForbiddenRuntimeCopy('保存高频模板题和关键知识点。', 'fixture.wxml').some((item) => item.pattern === '模板题'));
assert.ok(findForbiddenRuntimeCopy('放在知识点之后集中练。', 'fixture.wxml').some((item) => item.pattern === '集中练'));
...
assert.ok(runtimeFiles.some((file) => file.endsWith(path.join('data', 'subject-manifest.js'))));
assert.ok(runtimeFiles.some((file) => file.endsWith(path.join('packages', 'math', 'repository.js'))));
assert.ok(runtimeFiles.every((file) => !file.includes(`${path.sep}data${path.sep}`) || file.endsWith(path.join('data', 'subject-manifest.js'))));
