...
当前可由本地原创文件重新压缩的 source-managed 清单有 `231` 项；运行时共保留 `801` 条云图片引用，其中 `570` 条是没有本地原图的历史 cloud-only 路径。当前部署证据只能完整证明前 231 项，不能据此宣称全部运行时图片都已核验。

## 决策与范围

本批为 231 项 source-managed 资源建立“本地资源快照 -> 开发者工具签名验证批次 -> 本地无敏感证据 -> 严格发布门禁”的闭环，复用已部署的 `getImageTempUrls` 云函数，不新增用户可见页面、内容正文、账号、学习任务或云端学习数据。
...
4. 验证器拒绝环境不一致、计划过期、覆盖不全、重复 ID、非零状态、缺少临时链接状态和未知文件；成功时才允许把 231 项 source-managed 资源描述为已核验。
5. `check-release-readiness.js --require-device-evidence` 额外要求当前提交、未改动的资源输入和当前资源快照对应的云资源证据。默认质量矩阵只验证工具与契约，不要求真实云端证据。
...
- 不把暂时可读的文字降级写成“云图已验证”。
- 不把 570 条没有本地原图的历史 cloud-only 引用写成已由本地 source-managed 计划验证；它们需要独立的云端存量核验后才可声称全量运行时图片可用。
...
CloudAssetPlan {
  ...
  assets: [{
    ...
    width, height, bytes, sha256, sourceSha256
  }],
  batches: [CloudAssetPlanAsset[]]
}
...
CloudAssetEvidence {
  ...
  results: [{ fileID, status, hasTempFileURL }]
}
```
...
- 任一资源未返回成功状态；
- 计划没有覆盖当前资源清单。
- 本地原图、资源输入或 source-managed 资源集合与证据所标记的 Git 提交不一致。
...
- 231 项 source-managed 基线资源产生唯一的云路径、文件 ID、快照哈希和最多 50 项的批次；801 条运行时引用集合的指纹也被锁定，避免新路径被静默排除。
