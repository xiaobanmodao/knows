...
- 已在绑定正确 AppID 的 iPhone 14 Pro Max 模拟器项目实例验证四类图解、阅读显示、收藏和返回栈；Nexus 5 模拟器验证了同类图解、相邻导航、收藏夹重新打开和文字降级。
- 若开发者工具通过“已有项目直接打开”重新载入本工作树时未携带 `appid`，会由工具框架产生 `webapi_getwxaasyncsecinfo:fail appid missing`，不能作为页面源码错误或发布证据。后续真机与发布回归必须从绑定 AppID 的项目实例开始。
- 模拟器验证不替代 v1.10.1 的实体设备证据，且本版本不执行体验版上传、审核提交或正式发布。
