...
4. 将检查器及其契约测试接入默认 v1.11 质量矩阵，并让两份当前路线文档的实际检查数继续自动同步。
...
矩阵在运行时分包依赖检查之后依次执行语法契约和实际语法扫描。项目约定所有 `*.test.js` 都必须进入默认矩阵，因此本项会增加两个检查；`check-roadmap-document-consistency.test.js` 已从矩阵长度派生两份路线文档中的展示数量，因此将门禁接入矩阵后，更新文档数字是强制步骤。
