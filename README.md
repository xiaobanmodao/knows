...
阶段一只用于定位并手动补传 biology 路径，不是严格发布证明。具有项目 AppID 开发权限的人员可在已绑定 AppID 的微信开发者工具云存储面板按 `biology-plan.json` 操作；脚本不会自动上传。
...
阶段二才是严格发布证明，必须覆盖所有当前运行时云资源。全量计划生成会覆盖同目录的 `verify-in-devtools.js`，因此必须在第二阶段重新粘贴并运行该脚本；保存的 evidence 只含状态和 `hasTempFileURL` 标记，不含临时 URL。
...
node scripts/build-cloud-asset-deployment-plan.js \
  ...
  --commit "$(git rev-parse HEAD)"
# 在绑定 AppID 的开发者工具控制台重新运行 dist/cloud-asset-deployment/verify-in-devtools.js。
